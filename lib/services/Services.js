...
export async function saveControlMeta(ferma, dataset, controlor, linii) {
    const requestData = {
        ferma: ferma,
        dataset: dataset,
        datareal: new Date().toISOString(), // Ensure date is in a string format for logging
        definitiv: false,
        controlor: controlor,
    };
    ...
    try {
        ...
        console.log(error.message, JSON.stringify(requestData));
...
export async function deleteControls(linii, controlId) {
    console.log(linii, controlId, "delete and save");

    try {
        await axios.delete(`http://185.92.192.96/genetica/api/Control/${controlId}`, {headers: headers});
...
export async function getControls(controlId) {
    try {
        const response = await axios.get(`http://185.92.192.96/genetica/api/Control/ByContrId/${controlId}`, {headers: headers});
